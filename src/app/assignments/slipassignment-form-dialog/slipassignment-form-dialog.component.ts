...
export class SlipassignmentFormDialogComponent implements OnInit {
  ...
  title = 'Assign Slip';
  instruction = 'Edit an assignment using the fields below. If a field doesn’t apply to your assignment, leave it blank.';
  subtitle1 = 'Assign Slip Info';
  subtitle2 = '';
    ...
    const unusedSlipList = this.slipList.filter((slipObj: any) => {
      const matchedObj = this.list.find((obj) => {
        if (obj && typeof obj === 'object') {
          if (obj.slip === slipObj.slip && this.item.slip !== slipObj.slip)
          return true;
        }
      });
      if (!matchedObj) {
  ...
  setFormValue(item: any) {
    let slip = '';
    let vessel = '';
    let vesselAvailability: '0' | '1' | '' = '';

    if (item && typeof item === 'object') {
      slip = item.slip || '';
      vessel = this.HList.findProperty(this.vesselList, { property: 'id', value: item.vesselId }, ['vessel']) || '';
      vesselAvailability = this.HList.findProperty(this.availabilityList, { property: 'value', value: item.vesselAvailability }, ['text']) || '';
    }
